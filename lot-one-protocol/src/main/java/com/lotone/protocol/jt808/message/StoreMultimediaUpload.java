package com.lotone.protocol.jt808.message;

import lombok.Data;

/**
 * 存储多媒体数据上传 (0x0802)
 * 终端向平台上传存储的多媒体数据
 */
@Data
public class StoreMultimediaUpload {
    public static final int MSG_ID = 0x0802;

    /** 多媒体ID */
    private long mediaId;

    /** 多媒体类型（0=图像 1=音频 2=视频） */
    private int mediaType;

    /** 通道ID */
    private int channelId;

    /** 事件项编码 */
    private int eventCode;

    /** 位置信息（28字节，可选） */
    private LocationInfo location;

    /** 多媒体数据 */
    private byte[] mediaData;

    @Data
    public static class LocationInfo {
        private long alarmFlag;
        private long status;
        private long latitude;
        private long longitude;
        private int altitude;
        private int speed;
        private int direction;
        private String time;
    }

    public static StoreMultimediaUpload decode(byte[] body) {
        StoreMultimediaUpload msg = new StoreMultimediaUpload();
        if (body == null || body.length < 7) return msg;

        int offset = 0;
        // 多媒体ID (4字节)
        msg.setMediaId(((body[offset] & 0xFFL) << 24) | ((body[offset+1] & 0xFFL) << 16)
                | ((body[offset+2] & 0xFFL) << 8) | (body[offset+3] & 0xFFL));
        offset += 4;
        // 多媒体类型 (1字节)
        msg.setMediaType(body[offset++] & 0xFF);
        // 通道ID (1字节)
        msg.setChannelId(body[offset++] & 0xFF);
        // 事件项编码 (1字节)
        msg.setEventCode(body[offset++] & 0xFF);

        // 位置信息（28字节）
        if (offset + 28 <= body.length) {
            LocationInfo loc = new LocationInfo();
            loc.setAlarmFlag(readLong(body, offset)); offset += 4;
            loc.setStatus(readLong(body, offset)); offset += 4;
            loc.setLatitude(readLong(body, offset)); offset += 4;
            loc.setLongitude(readLong(body, offset)); offset += 4;
            loc.setAltitude(((body[offset] & 0xFF) << 8) | (body[offset+1] & 0xFF)); offset += 2;
            loc.setSpeed(((body[offset] & 0xFF) << 8) | (body[offset+1] & 0xFF)); offset += 2;
            loc.setDirection(((body[offset] & 0xFF) << 8) | (body[offset+1] & 0xFF)); offset += 2;
            // 时间 BCD (6字节)
            StringBuilder timeSb = new StringBuilder();
            for (int i = 0; i < 6; i++) {
                timeSb.append(String.format("%02d", (body[offset + i] >> 4) & 0x0F));
                timeSb.append(String.format("%02d", body[offset + i] & 0x0F));
            }
            loc.setTime(timeSb.toString());
            offset += 6;
            msg.setLocation(loc);
        }

        // 剩余为多媒体数据
        if (offset < body.length) {
            byte[] data = new byte[body.length - offset];
            System.arraycopy(body, offset, data, 0, data.length);
            msg.setMediaData(data);
        }

        return msg;
    }

    private static long readLong(byte[] body, int offset) {
        return ((body[offset] & 0xFFL) << 24) | ((body[offset+1] & 0xFFL) << 16)
                | ((body[offset+2] & 0xFFL) << 8) | (body[offset+3] & 0xFFL);
    }

    public String getMediaTypeName() {
        switch (mediaType) {
            case 0: return "图像";
            case 1: return "音频";
            case 2: return "视频";
            default: return "未知";
        }
    }

    public String getFileExtension() {
        if (mediaType == 0) return ".jpg";
        if (mediaType == 1) return ".mp3";
        if (mediaType == 2) return ".wmv";
        return ".bin";
    }
}
