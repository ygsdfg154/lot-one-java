  ...
  <div class="authority">
    ...
    <div class="gva-table-box">
      <div class="gva-btn-list">
        <el-button v-if="isSuperAdmin" type="primary" icon="plus" @click="addAuthority(0)"
          >新增角色</el-button
        >
      </div>
      <el-table
        ...
        <el-table-column align="left" label="操作" width="560">
          <template #default="scope">
            ...
            <el-button
              ...
              type="primary"
              link
              :disabled="scope.row.authorityId === currentUserAuthorityId"
    ...
    <el-drawer v-model="authorityFormVisible" :size="appStore.drawerSize" :show-close="false">
      ...
      >
        ...
        <el-form-item label="数据范围" prop="dataScope">
          <el-select v-model="form.dataScope" style="width: 100%" placeholder="请选择数据范围">
            <el-option
        ...
        <el-form-item v-if="form.dataScope === 'custom'" label="自定义范围" prop="deptIds">
          <el-select
            ...
            collapse-tags
            style="width: 100%"
            placeholder="请选择平台/部门"
          >
            <el-option
              v-for="item in deptScopeOptions"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            />
...
import { ref, nextTick, computed } from 'vue'
  ...
  const currentUserAuthorityId = computed(() => userStore.userInfo.authority?.authorityId)
  const isSuperAdmin = computed(() => currentUserAuthorityId.value === 1)
  ...
  getDictFunc('data_scope').then(opts => { if (opts?.length) dataScopeOptions.value = opts })
  const deptScopeOptions = ref([])
  const rules = ref({
    authorityId: [
  ...
  const loadDeptScopeOptions = async () => {
    const res = await getLotDeptList({ page: 1, pageSize: 1000 })
    if (res.code === 0) {
      deptScopeOptions.value = deptListToOptions(res.data?.list)
    }
  }
  loadDeptScopeOptions()
  ...
  const copyAuthorityFunc = (row) => {
    ...
    form.value.dataScope = row.dataScope || 'all'
      ...
      if (valid) {
        ...
        switch (dialogType.value) {
          ...
          case 'copy': {
            ...
            data.authority.deptIds = form.value.deptIds
