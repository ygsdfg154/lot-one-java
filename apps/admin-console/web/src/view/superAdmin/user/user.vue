    ...
    <div class="gva-search-box">
      <el-form ref="searchForm" :inline="true" :model="searchInfo">
        ...
        <!-- <el-form-item label="平台">
          <el-select v-model="searchInfo.platformId" clearable filterable placeholder="请选择平台">
            <el-option
    ...
    <div class="gva-table-box">
      ...
      <el-table :data="tableData" row-key="ID" :default-sort="{ prop: 'ID', order: 'descending' }" @sort-change="sortChange">
        ...
        <el-table-column
          ...
          prop="email"
        />
        <el-table-column align="left" label="归属部门" min-width="130"  show-overflow-tooltip>
          <template #default="scope">{{ scope.row.deptName || getOptionLabel(deptOptions, scope.row.deptId) }}</template>
        </el-table-column>
    ...
    >
      ...
      >
        ...
        <el-form-item label="归属部门" prop="deptId">
          <el-select v-model="userInfo.deptId" clearable filterable placeholder="请选择部门" style="width: 100%">
            <el-option
              v-for="item in deptOptions"
              :key="item.value"
              :label="item.label"
        ...
        <el-form-item label="用户角色" prop="authorityId">
          <el-cascader
            ...
            :options="authOptions"
            :show-all-levels="false"
            :disabled="dialogFlag === 'edit' && userInfo.ID === userStore.userInfo.ID"
            :props="{
              multiple: true,
              checkStrictly: true,
              label: 'authorityName',
...
<script setup>
  ...
  import { getLotDeptList } from '@/api/lot/lotDept'
  ...
  const getOptionLabel = (optionsRef, value) => {
    if (!optionsRef?.value || !Array.isArray(optionsRef.value)) return value || '-'
    const valueNumber = Number(value)
    const option = optionsRef.value.find(item => Number(item.value) === valueNumber)
    return option?.label || value || '-'
  }

  const toOrgOptions = deptListToOptions

  const setOrgOptions = async () => {
    const [platformRes, deptRes] = await Promise.allSettled([
      getLotDeptList({ page: 1, pageSize: 1000, orgType: 'platform' }),
      getLotDeptList({ page: 1, pageSize: 1000 })
    ])
    if (platformRes.status === 'fulfilled' && platformRes.value.code === 0) {
      platformOptions.value = toOrgOptions(platformRes.value.data?.list)
    }
    if (deptRes.status === 'fulfilled' && deptRes.value.code === 0) {
      const allDepts = deptRes.value.data?.list || []
      if (platformOptions.value.length === 0) {
        platformOptions.value = toOrgOptions(allDepts.filter(item => item.orgType === 'platform'))
      }
      deptOptions.value = toOrgOptions(allDepts.filter(item => item.orgType !== 'platform'))
    }
  }
    ...
    userForm.value.validate(async (valid) => {
      if (valid) {
        ...
        if (dialogFlag.value === 'edit') {
          if (req.ID === userStore.userInfo.ID) {
            delete req.authorityIds
            delete req.authorityId
          }
  ...
  const closeAddUserDialog = () => {
    ...
    userInfo.value.platformId = undefined
    userInfo.value.deptId = undefined
