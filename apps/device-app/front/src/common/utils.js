...
import '@/.unpacked/svc/4602.js'
import i from './config.js'
import o from '@/.unpacked/svc/3cf4.js'
...
export function date2local(e) {
  return e ? "Invalid date" == r(e).format("L") ? "" : r(e).format("L") : "";
}
export function datetime2local(e) {
  return e ? "Invalid date" == r(e).format("L") ? "" : r(e).format("YYYY/MM/DD HH:mm") : "";
}
export function datetime3local(e) {
  return e ? "Invalid date" == r(e).format("L") ? "" : r(e).format("MM-DD HH:mm") : "";
}
export function datetimeDiff(e, t) {
  if (!e) return "";
  t || (t = new Date());
  var n = r(e), a = r(t), i = r.duration(a.diff(n)), o = i.days(), s = i.hours(), u = i.minutes(), d = i.seconds();
  o && o + "\u5929 ";
  s && s + "\u5c0f\u65f6 ";
  u && u + "\u5206\u949f ";
  d && d + "\u79d2";
...
export function gcoordTransform(e) {
  if (e.lonlat[0] && e.lonlat[1]) {
    var t = {
      WGS84: o.WGS84,
      GCJ02: o.GCJ02
    }, n = o.transform(e.lonlat, t[e.to], t[e.form]);
    return n;
  }
  return e.lonlat;
}
export function getDays(t, n) {
  var a = r(t), i = r(n), o = r.duration(i.diff(a)), s = o.years(), u = o.months(), d = o.days();
  o = r.duration({
    years: s,
    months: u,
    days: d
  ...
  var c = o.asDays();
  return (e("log", c, "totalDays", " at utils/common.js:362"), c);
}
export function getNetworkTime(t, n, a) {
  e("log", t, n, "startstaet", " at utils/common.js:297");
  var i = r(t), o = r(n), s = r.duration(o.diff(i)), u = s.years(), d = s.months(), c = s.days(), l = s.hours(), f = s.minutes(), m = s.seconds();
  if (a) {
    if (u) return ("").concat(u, "\u5e74").concat(d, "\u6708").concat(c, "\u5929").concat(l, "\u5c0f\u65f6").concat(f, "\u5206").concat(m, "\u79d2");
    if (d) return ("").concat(d, "\u6708").concat(c, "\u5929").concat(l, "\u5c0f\u65f6").concat(f, "\u5206").concat(m, "\u79d2");
    if (c) return ("").concat(c, "\u5929").concat(l, "\u5c0f\u65f6").concat(f, "\u5206").concat(m, "\u79d2");
    if (l) return ("").concat(l, "\u5c0f\u65f6").concat(f, "\u5206").concat(m, "\u79d2");
    if (f) return ("").concat(f, "\u5206").concat(m, "\u79d2");
    if (m) return ("").concat(m, "\u79d2");
  } else {
    if (u) return ("").concat(u, "\u5e74 ").concat(d, "\u6708 ").concat(c, "\u5929");
    if (d) return ("").concat(d, "\u6708 ").concat(c, "\u5929");
    if (c) return ("").concat(c, "\u5929");
  }
}
/**
 * 按 ["省","市","区"] 逐级定位行政区节点（"不限"表示只选到上一级）。
 *
 * 子节点字段是契约 8.2.6 的 `children`（原来读高德的 `districts`）。找不到就返回空
 * 数组而不是抛错——区划每年调整，用户存的旧选择可能已经不存在了。
 */
export function getRegionInfo(e, t) {
  ...
  if (ANY == t[1] || !n.length) return n;
  var a = (n[0].children || []).filter(function (e) {
    return e.name == t[1];
  });
  if (ANY == t[2] || !a.length) return a;
  var r = (a[0].children || []).filter(function (e) {
    return e.name == t[2];
  });
  return r;
}
...
export function getUrlCode() {
  ...
  if (-1 != e.indexOf("?")) for (var n = e.substr(1), a = n.split("&"), r = 0; r < a.length; r++) t[a[r].split("=")[0]] = a[r].split("=")[1];
  return t;
}
 ...
 * 行政区边界 → 地图组件要的顶点数组。
 *
 * 契约 8.2.7 直接给 `{points:[{lat,lng}]}`，不再需要解析高德的 `polyline` 字符串
 * （原版按 `|` 拆多段、按 `;` 拆点、按 `,` 拆经纬度）。返回值仍是"多段"结构
 * （二维数组），因为 map 组件的 polygon 接口按段绘制。
 */
export function parseRegion(e) {
  var points = (e && e.points) || (Array.isArray(e) ? e : []);
  var seg = points.map(function (p) {
    return { longitude: Number(p.lng), latitude: Number(p.lat) };
  });
