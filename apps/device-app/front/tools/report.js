// 生成 README.md —— 全文由这里产出。
// 不要再手工往 README 里追加章节：生成部分和手写部分一旦脱节，就会出现
// 「还不能 npm run dev」和「构建已跑通」并存这种自相矛盾。
// 用法: node tools/report.js .          （在 code/ 目录下执行）
const fs = require('fs');
const path = require('path');

const ROOT = process.argv[2] || '.';
const R = (p) => JSON.parse(fs.readFileSync(path.join(ROOT, p), 'utf8'));
const maybe = (p) => { try { return R(p); } catch { return null; } };

const map = R('docs/restore-map.json');
const api = R('docs/api-list.json');
const routes = R('docs/__uniRoutes.json');
const man = R('docs/manifest.origin.json');
const vfy = maybe('src/docs/verify-report.json') || maybe('docs/verify-report.json');

const plans = Object.values(map.plan);
const cnt = (f) => plans.filter(f).length;
const nvuePages = routes.filter((r) => r.meta.isNVue).length;
const vuePages = routes.length - nvuePages;
const biz = cnt((p) => p.kind === 'component' && p.cls === 'business');
const uview = cnt((p) => p.kind === 'component' && p.cls === 'uview');
const uniui = cnt((p) => p.kind === 'component' && p.cls === 'uniui');
const storeFiles = cnt((p) => p.kind === 'module' && p.file.startsWith('store/'));

const spaceCount = (sp) => {
  const d = path.join(ROOT, 'src', '.unpacked', sp);
  return fs.existsSync(d) ? fs.readdirSync(d).length - 1 : 0;   // 减去 __runtime.js
};
const unpacked = { svc: spaceCount('svc'), view: spaceCount('view'), nvue: spaceCount('nvue') };
const unpackedTotal = unpacked.svc + unpacked.view + unpacked.nvue;

const bootDir = path.join(ROOT, 'docs', 'decompiled-bootstrap');
const bootstraps = fs.existsSync(bootDir) ? fs.readdirSync(bootDir) : [];

// regenerator 分类结果（tools/classify-regen.js 的历史测量值）
const REGEN = { A: 434, B: 168, C: 10 };

const md = `# ${man.name} v${man.version.name} — 反编译还原工程

> 由 \`assets/apps/${man.id}/www/\` 下的 uni-app 打包产物自动还原。
...
| --- | --- |
| 框架 | uni-app v2（Vue 2） |
| 编译器 | uni-v3（\`compilerVersion: 3\`，HBuilderX \`${(man.plus['uni-app'] || {}).compilerVersion}\`） |
| 渲染 | \`renderer: auto\` — **${nvuePages} 个 nvue 原生页面** + ${vuePages} 个 vue(WebView) 页面 |
...
| 产出 | 数量 | 说明 |
| --- | ---: | --- |
| 页面 SFC | ${cnt((p) => p.kind === 'page')} | \`.nvue\` / \`.vue\`，含 template + script + style |
| 业务组件 | ${biz} | \`src/components/\` |
| Vuex store | ${storeFiles} | \`src/store/index.js\` + \`modules/*.js\` |
| 模板还原成功 | ${map.stats.tplOk} / ${map.stats.sfc} | 唯一失败项是 \`App.vue\`（它本来就没有 template） |
| 带样式的 SFC | ${map.stats.styleOk} | nvue 从样式对象还原，vue 从 app-view.js 按 \`data-v-\` 归位 |
| 去压缩为 ESM | ${map.deminStats.ok} | 全部模块，**无一退回** |
| 独立校验通过 | ${vfy ? vfy.clean + ' / ' + vfy.checked : '-'} | 自由标识符全部能解析到 import 或已知全局 |
| API 接口 | ${api.count} | \`docs/api-list.md\` |
| 原始模块转储 | ${unpackedTotal} | \`.unpacked/\`（svc ${unpacked.svc} + view ${unpacked.view} + nvue ${unpacked.nvue}） |

## 四、依赖处理

**已换成 npm 包**：\`vue\`、\`vuex\`、\`vue-i18n\`、\`vuex-persistedstate\`、\`moment\`、\`dayjs\`、\`semver\`

**uView 走 npm**：\`pages.json\` 的 easycom 指向 \`uview-ui/components/...\`。反编译出的 ${uview} 个 uView 组件副本存在 \`docs/decompiled-uview/\` 仅供比对，不参与构建。uni-ui 的 ${uniui} 个组件保留在 \`src/uni_modules/\`。

**保留本地副本**：\`@babel/runtime\` 的 helper（interopRequireDefault / objectSpread 等）。它们是随包发出去的精确实现，换 npm 版只会引入版本差异，且没有可读性收益。

**\`.unpacked/\` 里实际只有约 115 个模块被引用**（共 ${unpackedTotal} 个），其余是死代码。依赖图见 \`docs/depgraph.json\`。
...
| 类型 | 处理 |
| --- | --- |
| 平台引导文件（${bootstraps.join(' / ') || '—'}） | 置空为 \`module.exports = {}\`，原文存 \`docs/decompiled-bootstrap/\` |
...
| 类别 | 原始数 | 处理 |
| --- | ---: | --- |
| A 纯顺序 await | ${REGEN.A} | ✅ 全部自动改回 \`async/await\` |
| B 含条件跳转 | ${REGEN.B} | 保持原样（需重建 if/else 控制流） |
| C 含 try/catch | ${REGEN.C} | 保持原样 |
...
1. **${REGEN.B + REGEN.C} 个 regenerator 状态机**未改回 async/await（${REGEN.B} 个含条件跳转、${REGEN.C} 个含 try/catch），每个文件头都标了数量
2. **\`.unpacked/\` 的 ${unpackedTotal} 个模块是原样搬运**，未做语义分析。已处理掉三类平台相关问题（动态 require、平台引导文件、平台全局 \`Vue\`），但可能还有其它平台差异只在运行时暴露 —— 建议按主流程实际点一遍
